...
Defines how the primary site navigation (brand entry plus the link list from `NavConfig`) is presented across viewport sizes by the shared `AppBar` component. The capability covers responsive collapse, single-source link rendering, active-state semantics, and SSR stability.
