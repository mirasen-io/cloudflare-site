# license-gate-page Specification

## Purpose

Defines the `/license-gate` landing page that presents the Mirasen License Gate open-source engineering tool. The capability covers the page route and section skeleton, hero and calls to action, the Quick Start snippet, the policy-model trust story, scope-boundary framing, neutral outbound links, page-local navigation, home-page discoverability, metadata and JSON-LD, sitemap inclusion, and the accompanying changeset entry.
