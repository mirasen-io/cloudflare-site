...
- **New code**: `src/routes/license-gate/+page.svelte`, `src/routes/license-gate/+layout.ts`.
- **Modified code**: `src/lib/nav/links.ts` (add `licenseGateNav` export; `rootNav.links` and `chessboardNav.links` untouched); `src/routes/+page.svelte` (add small secondary "Engineering tools" mention below existing sections).
- **Untouched**: `src/lib/components/AppBar.svelte`; existing `/chessboard` page and its nav wiring; `/chess-lore` page; footer; layout CSS.
