...
- [ ] 8.2 Re-inspect `../license-gate/CHANGELOG.md` at implementation time per design.md D6.1. Confirm it still contains substantive release content (the `## 1.0.0` entry describing the initial v1 surface). Record the outcome as an inline comment or task-completion note.
...
- [ ] 9.3 Single `article.card.p-6` (not a `.card-grid`, not `.spotlight`), containing `h3.card-title "Mirasen License Gate"` and one paragraph `"A strict local license policy gate for npm projects. Built for default-deny CI checks."`.
...
- [ ] 13.1 Load `/` and confirm the new "Engineering tools" section appears **after** "One brand, multiple layers" and reads as a subordinate note, not an equal fourth product pillar. The single card is visibly narrower than the existing `.card-grid.three-up` and does not use `.spotlight`.
...
- [ ] 14.3 Run `npm run build`. Expect a prerendered `/license-gate/index.html` in the static output. Grep it for the literal string `Mirasen License Gate` (must be present) and for the canonical URL `https://mirasen.io/license-gate` (must be present).
- [ ] 14.4 Run `npm run test:unit -- --run`. Expect existing tests to pass; no new unit tests are added by this change. No new Playwright e2e is added for this content-only page.

## 15. Cleanup

- [ ] 15.1 Verify final `git diff` touches only: `src/lib/nav/links.ts` (additions), `src/routes/license-gate/+page.svelte` (new), `src/routes/license-gate/+layout.ts` (new), `src/routes/+page.svelte` (one appended section).
