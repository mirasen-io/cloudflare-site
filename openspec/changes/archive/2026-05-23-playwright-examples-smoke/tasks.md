...
- [x] 1.1 In `src/routes/+layout.svelte`, import `onMount` from `svelte` and call `document.documentElement.classList.add('app-started')` inside it. No SSR side effects, no data attribute, no body class.
...
- [x] 2.1 In `src/routes/chessboard/examples/live-games-grid/LiveCard.svelte`, add `data-testid="live-game-card"` to the root `<article class="live-card">`.
- [x] 2.2 In the same file, add `data-testid="live-board-link"` to the overlay `<a class="live-card__overlay">`.
- [x] 2.3 Confirm no other example pages need new `data-testid` attributes — controls are reachable by accessible button name and board mount is reachable via `[data-chessboard-id="board-root"]`.

## 3. Page-error helper

- [x] 3.1 Create `e2e/utils/page-errors.ts` exporting `trapPageErrors(page: Page): () => void` that:
...
- [x] 4.1 Create `e2e/chessboard-examples/minimal.e2e.ts`:
...
- [x] 4.2 Create `e2e/chessboard-examples/promotion.e2e.ts`:
  ...
  - call `assertNoPageErrors()` as the final step.
- [x] 4.3 Create `e2e/chessboard-examples/chessjs.e2e.ts`:
  - install `const assertNoPageErrors = trapPageErrors(page);`
  - navigate, wait for `html.app-started`
  ...
  - call `assertNoPageErrors()` as the final step.
- [x] 4.4 Create `e2e/chessboard-examples/live-games-grid.e2e.ts`:
  - install `const assertNoPageErrors = trapPageErrors(page);`
  - navigate, wait for `html.app-started`
...
- [x] 5.1 Confirm no inbound link to `/demo` or `/demo/playwright` from `src/lib/nav/links.ts`, the example pages, or any other production code.
- [x] 5.2 Delete `src/routes/demo/playwright/page.svelte.e2e.ts`.
- [x] 5.3 Delete `src/routes/demo/playwright/+page.svelte` and `src/routes/demo/+page.svelte`.
- [x] 5.4 Remove the now-empty `src/routes/demo/` directory.
- [x] 5.5 Verify no other `*.e2e.ts` remains under `src/routes/`.

## 6. Validation

- [x] 6.1 `npm run lint` passes (Prettier + ESLint).
- [x] 6.2 `npm run check` passes (svelte-kit sync + svelte-check).
- [x] 6.3 `npm run build` passes.
- [x] 6.4 `npm run test:e2e` passes locally — Playwright builds, previews on `:4173`, and runs all four new smoke specs end-to-end. Confirm the demo `*.e2e.ts` is no longer reported and the four new specs are.
...
- [x] 7.1 Add a one-paragraph note to `e2e/README.md` (create if absent) explaining the `app-started` hydration convention, the `[data-chessboard-id="board-root"]` board-mount convention, and where to add new example smoke specs. Do not duplicate the spec; link to `openspec/specs/chessboard-examples-smoke/spec.md`.
